...
static inline ULONG_T IOC_diffTimeSpecInMS(const struct timespec *pFromTS, const struct timespec *pToTS) {
    if (pToTS->tv_nsec < pFromTS->tv_nsec) {
        // 如果pToTS的纳秒小于pFromTS的纳秒，需要从秒数中借位
        return (pToTS->tv_sec - pFromTS->tv_sec - 1) * 1000 + ((pToTS->tv_nsec + 1000000000) - pFromTS->tv_nsec) / 1000000;
    } else {
        // 正常情况下的计算
        return (pToTS->tv_sec - pFromTS->tv_sec) * 1000 + (pToTS->tv_nsec - pFromTS->tv_nsec) / 1000000;
    }
}
