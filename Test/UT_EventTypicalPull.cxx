...
TEST(UT_ConetEventTypical, verifyPullEVT_byInfiniteTimeout_expectEventualSuccess) {
    ...
    // Start thread to post event after delay - testing infinite timeout patience
    std::atomic<bool> EventPosted{false};
    std::thread PostThread([&] {
        std::this_thread::sleep_for(std::chrono::milliseconds(DelayMS));
        IOC_EvtDesc_T PostedEvt = {};
        PostedEvt.EvtID = IOC_EVTID_TEST_KEEPALIVE;
        PostedEvt.EvtValue = 300;
    ...
    // Verify successful event reception and timing behavior
    ASSERT_EQ(IOC_RESULT_SUCCESS, ResultValue) << "Should successfully receive event with infinite timeout";
    ASSERT_EQ(IOC_EVTID_TEST_KEEPALIVE, IOC_EvtDesc_getEvtID(&PulledEvt));
    ...
    ASSERT_LE(actualDuration.count(), (int)(DelayMS * 3.0)) << "Should not have waited excessively long";
